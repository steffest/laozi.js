/*
    ...
    Based partially on "multiparty"
     ...
     Adapted by Steffest to remove dependencies
...
Form.prototype.parse = function(req, cb) {
    ...
    if (!contentType) {
        validationError(415); //missing content-type header
        return;
    }

    var m = CONTENT_TYPE_RE.exec(contentType);
    if (!m) {
        validationError(415) // unsupported content-type
        return;
    }
    ...
    if (!boundary) {
        validationError(400) //content-type missing boundary
    }
        ...
        switch (state) {
            ...
            case START_BOUNDARY:
                if (index === boundaryLength - 2 && c === HYPHEN) {
                    ...
                    if (c !== CR) return self.handleError(400) // Expected CR Received
                    index++;
                    break;
                } else if (index === boundaryLength - 1) {
                    if (c !== LF) return self.handleError(400) // Expected LF Received
                    index = 0;
            ...
            case HEADER_FIELD:
                ...
                if (c === COLON) {
                    if (index === 1) {
                        // empty header field
                        self.handleError(400) // Empty header field
                        return;
                    }
                ...
                cl = lower(c);
                if (cl < A || cl > Z) {
                    self.handleError(400); //Expected alphabetic character
                    return;
                }
                break;
            ...
            case PART_DATA:
                ...
                break;
            case CLOSE_BOUNDARY:
                if (c !== HYPHEN) return self.handleError(400);  // Expected HYPHEN Received
                if (index === 1) {
                    self.onParsePartEnd();
                    state = END;
                } else if (index > 1) {
...
Form.prototype.onParseHeadersEnd = function(offset) {
    ...
    switch(self.partTransferEncoding){
        ...
        case 'base64': break;
        default:
            return 400 //  // unknown transfer-encoding: ' + self.partTransferEncoding);
    }

    self.totalFieldCount += 1;
    if (self.totalFieldCount > self.maxFields) {
        return 413; // maxFields ' + self.maxFields + ' exceeded.'
    }
...
function handleField(self, fieldStream) {
    ...
    fieldStream.on('readable', function() {
        ...
        if (self.totalFieldSize > self.maxFieldsSize) {
            self.handleError(413); // 'maxFieldsSize ' + self.maxFieldsSize + ' exceeded'
            return;
        }
...
function setUpParser(self, boundary) {
    ...
    self.on('finish', function() {
        if (self.state !== END) {
            self.handleError(400); //stream ended unexpectedly
        }
        endFlush(self);
    });
}
...
function uid(){
    ...
    for (var i = 0; i < generateAttempts; i++) {
        try {
            return toString(crypto.randomBytes(18));
        } catch (e) {
            err = e;
        }
    }
